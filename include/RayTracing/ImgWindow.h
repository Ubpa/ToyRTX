...
namespace RayTracing {
	...
	public:
		enum ENUM_OPTION {
			ENUM_OPTION_BLUR = 1 << 0,
			ENUM_OPTION_SAVE_SRC_IMG = 1 << 1,
			ENUM_OPTION_SAVE_POST_PROCESS_IMG = 1 << 2,
			ENUM_OPTION_SAVE_ALL_IMG = ENUM_OPTION_SAVE_SRC_IMG | ENUM_OPTION_SAVE_POST_PROCESS_IMG,
			ENUM_OPTION_ALL = ENUM_OPTION_BLUR | ENUM_OPTION_SAVE_ALL_IMG,
		};
		ImgWindow(const std::string & title = "Image_Window", size_t fps = 60, ENUM_OPTION option = ENUM_OPTION_ALL);
