...
Scene::CPtr CreateScene8(float ratioWH) {
	...
	tfmBottom = rotate(tfmBottom, -Math::PI / 2, vec3(1, 0, 0));
	tfmBottom = scale(tfmBottom, vec3(24));
	...
	auto sphere0 = ToPtr(new Sphere(vec3(-4, 2, 2), 2, ToCPtr(new Metal(vec3(1.0)))));
	auto sphere1 = ToPtr(new Sphere(vec3(-6, 2, 6), 2, ToCPtr(new Lambertian(vec3(Math::Rand_F()*Math::Rand_F(), Math::Rand_F()*Math::Rand_F(), Math::Rand_F()*Math::Rand_F())))));
	auto sphere2 = ToPtr(new Sphere(vec3(4, 2, -2), 2, ToCPtr(new Dielectric(1.5f))));

	auto bunny = ToCPtr(new Model(rootPath + str_Obj_Bunny));
	mat4 tfmBunnyMat(1.0f);
	tfmBunnyMat = translate(tfmBunnyMat, vec3(0, 3.5, 0));
	tfmBunnyMat = rotate(tfmBunnyMat, -Math::PI / 2, vec3(0, 1, 0));
	tfmBunnyMat = scale(tfmBunnyMat, vec3(0.004));
	...
	mat4 fit2Cube(1.0);
	fit2Cube = scale(fit2Cube, vec3(1.0f/dragonModel->GetBoundingBox().GetRadius()));
	fit2Cube = translate(fit2Cube, - dragonModel->GetBoundingBox().GetCenter());

	mat4 tfmDragonGlass = mat4(1.0);
	tfmDragonGlass = translate(tfmDragonGlass, vec3(-5, 2, -3));
	...
	//tfmDragonVolumeModel = rotate(tfmDragonVolumeModel, -Math::PI / 2, vec3(0, 1, 0));
	tfmDragonVolumeModel = scale(tfmDragonVolumeModel, vec3(3.95));
	tfmDragonVolumeModel *= fit2Cube;
	auto dragonVolumeModel = ToCPtr(new Transform(tfmDragonVolumeModel, dragonModel));

	auto dragonVolume = ToPtr(new Volume(dragonVolumeModel, 16.0f, ToPtr(new Isotropic(rgb(0.2f, 0.4, 0.9f)))));
	auto dragon = ToPtr(new Group);
	(*dragon) << dragonGlass << dragonVolume;

	//mat4 tfmDragonMat(1.0f);
	//tfmDragonMat = translate(tfmDragonMat, vec3(-5, 1, -3));
	//tfmDragonMat = rotate(tfmDragonMat, -Math::PI / 2, vec3(0, 1, 0));
	//tfmDragonMat = scale(tfmDragonMat, vec3(0.4));
	//auto tfmDragon = ToCPtr(new Transform(tfmDragonMat, dragon, ToCPtr(new Metal(vec3(1.0), 0.5f))));

	auto group = ToPtr(new Group);
	auto group1 = ToPtr(new Group);
	(*group) << sphere0 << sphere1 << sphere2 << bottomWood << sky << tfmBunny << dragon;

	vec3 origin(-4, 6, -10);
	vec3 viewPoint(0, 2, 0);
	float fov = 60.0f;
	float lenR = 0.00f;
	//float distToFocus = 10.0f;
