...
void main(){
	...
	//	out_rayTracingRst=vec3(At(MatData, TexCoords.x * textureSize(MatData, 0).x),0,0)/6;
	//else
	//	out_rayTracingRst=vec3(At(SceneData, TexCoords.x * textureSize(SceneData, 0).x),0,0);
	//out_rayTracingRst = vec3(At(SceneData, int(TexCoords.x*textureSize(SceneData, 0).x+TexCoords.y*textureSize(SceneData, 0).y*textureSize(SceneData, 0).x)),0,0);
	//float SceneDataIdx = int(TexCoords.x*textureSize(SceneData, 0).x)+int(TexCoords.y*textureSize(SceneData, 0).y)*textureSize(SceneData, 0).x;
	//out_rayTracingRst = vec3(At(SceneData, SceneDataIdx), 0, 0);
	float PackDataIdx = int(TexCoords.x*textureSize(PackData, 0).x)+int(TexCoords.y*textureSize(PackData, 0).y)*textureSize(PackData, 0).x;
	vec4 tmp;
	//GetPack(PackDataIdx, tmp);
	//out_rayTracingRst = vec3(tmp[3],0,0);
	RayTracer();
}
