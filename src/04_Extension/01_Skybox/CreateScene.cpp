...
Scene::CPtr CreateScene5(float ratioWH) {
	...
	auto noiseMat = ToPtr(new Lambertian(OpTexture::NoiseTexture(1, vec3(1), 2.0f)));
	auto bottomWall = ToPtr(new Transform(tfmBottom, square, noiseMat));

	auto sphere0 = ToPtr(new Sphere(vec3(-2, 1, 1), 1, ToCPtr(new Metal(vec3(1.0)))));
	auto sphere1 = ToPtr(new Sphere(vec3(0, 1, 0), 1, ToCPtr(new Lambertian(vec3(Math::Rand_F()*Math::Rand_F(), Math::Rand_F()*Math::Rand_F(), Math::Rand_F()*Math::Rand_F())))));
	auto sphere2 = ToPtr(new Sphere(vec3(2, 1, -1), 1, ToCPtr(new Dielectric(1.5f))));

	auto group = ToPtr(new Group);
	(*group) << sky << sphere0 << sphere1 << sphere2 << bottomWall;
