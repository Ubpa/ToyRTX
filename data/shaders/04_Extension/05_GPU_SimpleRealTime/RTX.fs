...
layout (location = 0) out vec3 out_SumColor;
...
const float rayP = 40.0/41.0;// depth = p/(1-p) --> p = depth/(depth+1)
...
vec3 RayTracer(struct Ray ray);

void main(){
	struct Ray ray;
	Camera_GenRay(ray);
	out_SumColor = RayTracer(ray) + (1-clear)*texture(SumColor, TexCoords).xyz;
}
...
vec3 RayTracer(struct Ray ray){
	//if(Rand()<0.5)
	//	return curLoop == 0? vec3(0) : texture(SumColor, TexCoords).xyz / curLoop;
